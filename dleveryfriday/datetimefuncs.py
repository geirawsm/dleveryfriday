...
import pendulum
import re
from dleveryfriday import _vars
...
def make_dt(date_in):
    # TODO Check and make some tests
    '''
...
def get_dt(format='epoch', sep='.', dt=False):
    ...
    if format == 'date':
        ...
        return dt.format(f'dddd')
    elif format == 'dayofweek':
        return dt.format(f'E')
    elif format == 'epoch':
...
def is_date1_more_than_date2(date1, date2):
    date1 = get_dt(format='epoch', dt=make_dt(date1))
    date2 = get_dt(format='epoch', dt=make_dt(date2))
    if date1 > date2:
        return True
    else:
        return False
...
if __name__ == "__main__":
    #pass
    print(get_dt(format='dayofweek', dt=make_dt('17.05.2014, 14.23')))
